function renderMoon(canvas, data) {
    ...
    // --- корректировка NEW MOON ---
    // если фаза "NEW" и освещённость > 0%, считаем это растущим месяцем
    if (apiPhase.includes('NEW')) {
        renderPhase = 'WAXING_CRESCENT';
    }
    ...
    if (renderPhase === 'WAXING_CRESCENT' || renderPhase === 'WANING_CRESCENT') {
        ...
        const bend = (t - 0.5) * 0.2 * r; // слабый изгиб
